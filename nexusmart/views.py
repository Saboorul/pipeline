...
def search_product(request):  # sourcery skip: none-compare
    ...
    try:
        if cache.get(search):
            ...
            if search:
                products = Product.objects.all().filter(
                    Q(is_available=True) & Q(product_name__contains=search) | Q(category__category_name__contains=search)
                )
        ...
        products = Product.objects.all().filter(
            Q(is_available=True) & Q(product_name__contains=search)
            | Q(category__category_name__contains=search)
            | Q(product_name=search)
        )

    products_count = products.count()
    context = {"products": products, "search": search, "p_count": products_count}
...
def update_cart(request):
    ...
    if request.headers.get("x-requested-with") == "XMLHttpRequest":
        ...
        if quantity != None and product_id != None:
            ...
            update_cartItems = CartItems.objects.filter(cart__cart_id=session_id, product__id=product_id).update(
                quantity=quantity, total=subtotal
            )
